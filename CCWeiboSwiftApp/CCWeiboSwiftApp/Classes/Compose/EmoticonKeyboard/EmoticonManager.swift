...
class EmoticonManager: NSObject {
    ...
    func appendRecentEmoticon(emoticon: EmoticonModel) {
        ...
        let array = emoticonArray?.sorted(by: { (a, b) -> Bool in
    ...
    class func emoticonMutableAttributedString(string: String, font: UIFont) -> NSMutableAttributedString {
        ...
        let resultArray = regularExpression.matches(in: string, options: NSRegularExpression.MatchingOptions(rawValue: 0), range: NSRange(location: 0, length: string.characters.count))
        let attributedString = NSMutableAttributedString(string: string)
        for result in resultArray.reversed() {
            let chs = (string as NSString).substring(with: result.range)
            let pngPath = findPngPath(chs: chs)
            ...
            attributedString.replaceCharacters(in: result.range, with: emoticonAttributedString)
        }
